...
pub fn run() -> Result<()> {
    ...
    let cli = Cli::parse();
    // Install the user palette + key maps before any TUI renders. Cheap
    // (one small file read) and uniform across commands.
    let cfg = Config::load();
    ui::theme::init(&cfg.theme);
    ui::keys::init(&cfg.keys);
    drop(cfg);
    match cli.command {
        None => run_selector(),
...
fn run_agents(json: bool) -> Result<()> {
    if json {
        ...
        proc.refresh();
        let snapshot = agents::collector::collect(&proc, &Config::load().agents);
